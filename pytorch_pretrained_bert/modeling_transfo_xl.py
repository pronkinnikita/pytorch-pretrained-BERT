...
class TransfoXLClassificationHeadModel(TransfoXLPreTrainedModel):
    def __init__(self, config, clf_dropout=0.4, n_class=8):
        super(TransfoXLClassificationHeadModel, self).__init__(config)
        self.transformer = TransfoXLModel(config)
        self.dropout = nn.Dropout(clf_dropout)
        self.linear = nn.Linear(config.d_embed * 2, n_class)
    ...
    def forward(self, input_ids, target=None, mems=None):
        ...
        last_hidden, _ = self.transformer(input_ids, mems)
